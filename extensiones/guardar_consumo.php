<?php
  ...
  $medida	      = $conexion->real_escape_string($_POST['medida']);
  $id_usuario	  = $conexion->real_escape_string($_POST['id_consumidor']);

	if(($cantidad != "") and ($orden != "") and ($descripcion != "") and ($ubicacion != "") and ($medida != "")){
    ...
    $actualizarrecuento   = mysqli_query($conexion, "UPDATE tabla_recuento set consumo = (consumo + $cantidad),resultado = (inventario - $cantidad) where id_articulo = $id_articulo");
    ...
    // $actualizameses       = mysqli_query($conexion, "UPDATE `tabla_consumos$corteEnCurso` set `$meses[$mes_hoy]` = (`$meses[$mes_hoy]` + $cantidad),total = (total + $cantidad)where id_articulo = $id_articulo");
